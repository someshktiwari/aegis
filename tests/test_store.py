...
@pytest.mark.asyncio
async def test_insert_pending_creates_record(db):
    await insert_pending(db, "key-001", "fp-aaa")
    record = await get_record(db, "key-001")

    assert record is not None
    assert record.key == "key-001"
    assert record.fingerprint == "fp-aaa"
    assert record.status == KeyStatus.PENDING
...
async def test_delete_record_removes_row(db):
    await insert_pending(db, "key-003", "fp-ccc")
...
async def test_delete_expired_removes_old_rows(db):
    ...
    await db.execute(
        "INSERT INTO idempotency_keys (key, fingerprint, status, created_at) VALUES (?, ?, 'COMPLETE', ?)",
...
async def test_delete_expired_preserves_fresh_rows(db):
    await insert_pending(db, "fresh-key", "fp-fresh")
    deleted = await delete_expired(db)
    assert deleted == 0
